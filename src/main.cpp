...
int main(int argc, char** argv) {
  ...
  // argv[0] so the engine can find a network sitting beside it. A GUI launches
  // by full path, which is the case this is for; launched through PATH there
  // is no directory in it and the search falls back to the working directory.
  luna::UsiEngine engine(argc > 0 ? argv[0] : "");
